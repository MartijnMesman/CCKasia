...
export default function AudioPlayer({ src, title, className = '' }: AudioPlayerProps) {
  ...
  useEffect(() => {
    ...
    const audioSrc = convertDropboxUrl(src)
    audio.src = audioSrc
    ...
    const handleError = () => {
      setError('Audio kon niet worden geladen. Probeer de download optie.')
      setIsLoading(false)
    }
  ...
  const togglePlayPause = async () => {
    ...
    try {
      ...
      setError('Afspelen mislukt. Probeer opnieuw of gebruik de download optie.')
    }
  }
  ...
  const handleDownload = () => {
    ...
    link.download = title || 'audio-file.mp3'
    link.target = '_blank'
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-900/20 border border-red-500/30 rounded-lg">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h4 className="text-red-300 font-semibold text-sm mb-1">Audio Probleem</h4>
